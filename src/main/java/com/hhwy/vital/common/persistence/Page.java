...
public class Page<T> {
	...
	public Page(HttpServletRequest request, HttpServletResponse response, int defaultPageSize){
		...
		if (StringUtils.isNumeric(no)){
			CookieUtils.setCookie(response, "pageIndex", no);
			this.setPageIndex(Integer.parseInt(no));
		}else if (request.getParameter("repage")!=null){
			no = CookieUtils.getCookie(request, "pageIndex");
			if (StringUtils.isNumeric(no)){
				this.setPageIndex(Integer.parseInt(no));
			}
		}
	...
	public Page(int pageIndex, int pageSize, long count) {
		this(pageIndex, pageSize, count, new ArrayList<T>());
	}
	 ...
	 * @param list 本页数据对象列表
	 */
	public Page(int pageIndex, int pageSize, long count, List<T> list) {
		this.setCount(count);
	...
	public String toString() {
		...
		sb.append(funcName+"("+pageIndex+",this.value,'"+funcParam+"');\" onclick=\"this.select();\"/> 条，");
...
//		sb.insert(0,"<div class=\"page\">\n").append("</div>\n");
	...
	/**
	 * 获取点击页码调用的js函数名称
	 * function ${page.funcName}(pageIndex){location="${ctx}/list-${category.id}${urlSuffix}?pageIndex="+i;}
	 * @return
	 */
	@JsonIgnore
...
//			}
//		}
//		return new PageRequest(this.pageIndex - 1, this.pageSize, new Sort(orders));
//	}
//	
//	/**
