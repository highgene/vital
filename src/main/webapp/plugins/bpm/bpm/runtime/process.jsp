...
<div class="row v_m_lr_15">
	<div class="scrollable" data-size="580" data-position="left">
		<div class="col-sm-12">
			...
			<fieldset>
				<legend>当前:<span id="currentTask"></span></legend>
				<form id="saveForm" class="form-horizontal" role="form">
					...
					<div class="form-group" style="display:none" id="nextReciverItemDiv">
						<div class="col-sm-12">
							<label class="col-sm-2 control-label">接收人</label>
							<div id="nextReceiverDiv" class="col-sm-10">
								<select class="form-control" id="nextReceiver" class="chosen-select" data-placeholder="请选择任务接收人"
					...
					<div class="form-group" >
						<div class="col-sm-12">
							<label class="col-sm-2 control-label"><a href="#" onclick="showOpinion()">填写备注</a></label>
							<div class="col-sm-10" id="opinionDiv" style="display:none">
								<textarea class="form-control" id="opinion" name="opinion" class="form-control"
									rows=6></textarea>
							</div>
						</div>
					</div>
...
<script type="text/javascript">
	...
	$(function() {
		...
		$.ajax({
			...
			error : function(xhr) {
				var msg = "初始化流程状态失败！",errMsg="";
				try{errMsg = JSON.parse(xhr['responseText'])['exceptionMsg'];}catch(e){};
				if(errMsg && errMsg.length) msg = errMsg;
				$.alert("error", msg);
			}
		});
	...
	function showOpinion(){
		...
		var dialog = window.parent.$("#processDialog");
		dialog.find(".modal-body").height(400);
		dialog.find("iframe").height(400-15);
	}
	//执行流程
	...
	function initForm(data) {
		if(data.currentTasks[0].length == 0){
