			...
			<div class="v_box" id="v_box_header2">
				<div class="widget-header ui-sortable-handle">
					<div class="widget-toolbar no-border" style="z-index: 10000">
					 <% if("2".equals(com.hhwy.framework.configure.ConfigHelper.getConfig("PROCESS.MSG.TYPE"))){%>
						<a href="javascript:void(0)" onclick="msgConfig('${param.id}')">
							<span class="btn btn-xs btn-primary"> <i
								class="ace-icon fa fa-comment-o bigger-130"></i> <span
								class="bigger-110">消息配置</span>
						</span></a>
					<%} %>
...
<script type="text/javascript">
	...
	function queryConfig() {
		$("#configGrid")
				.dataGrid(
						{
							checkbox : false,
							url : basePath + "bpm/bpm/config/taskdeinitionList/"
									+ definitionId,
							type : "get",
							align : "center",
							pagination : false,
	...
	function saveConfig(id, name, version) {
		$.ajax({
			url : basePath + "bpm/bpm/config/taskconfig/",
			type : 'post',
	...
	function initForm(result) {
		...
		for (var i = 0; i < selects.length; i++) {
			...
			var url = basePath + "bpm/bpm/config/userList";
	...
	function setType(type, id, defaultVal){
		var url = basePath + "bpm/bpm/config/";
		if(type == 0){
			url += "userList";
		}else{
			url += "roleList";
	...
	function configMessage(taskKey, definitionId, taskName){
		$.openWindow({
			...
			height : 500,
			destroy : true,
			url : basePath + "view/bpm/bpm/config/messageConfig?taskKey=" + taskKey+"&definitionId="+definitionId,
			title : "消息配置 【"+taskName+"】",
			showDefaultButton : false
		});
		
	}
	function msgConfig(definitionId){
		location = basePath + "view/bpm/bpm/config/messageConfigList?definitionId=" + definitionId;		
	}
	//框架JS
	mainH();
	function mainH() {
