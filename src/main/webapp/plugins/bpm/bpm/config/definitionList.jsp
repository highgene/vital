...
function queryConfig() {
	...
	$("#configGrid")
		...
		{
			checkbox : false,
			url : basePath + "bpm/bpm/config/definitionList/",
			type : "get",
			align : "center",
			pagination : true,
			rownum : false,
			...
			columns : [
					...
					},
					{
						...
						title : "流程名称",
						render : function(row) {
							return '<a target="_blank" href="${Config.baseURL}view/bpm/bpm/config/definitionImg?id='+row.id+'">'
									+ row.name + '</a>';
						}
					},
					{
					...
					},
					{
						...
						render : function(row) {
							return '<div class="hidden-sm hidden-xs action-buttons">'
									+ '<a class="blue" title="在线编辑" href="'+basePath+ 'bpm/bpm/model/listModel?processDefinitionId='+ row.id+ '&pageSize=10&pageIndex=0"><i class="ace-icon fa fa-edit bigger-130"></i></a>'
									+ '<a class="blue" title="配置任务" href="'+basePath+ 'view/bpm/bpm/config/taskList?id='+ row.id+ '"><i class="ace-icon fa fa-gear bigger-130"></i></a>'
									...
									+ '&nbsp;&nbsp;<a class="green" title="查看【'+ row.name+ '】的实例" href="'+basePath+ 'view/bpm/bpm/config/instanceList?id='+ row.id
...
function addConfig() {
	$.openWindow({
		...
		height : 280,
		destroy : true,
		url : basePath + "view/bpm/bpm/config/deploy",
		title : "部署流程",
...
$(function() {
	$('#createModel').click(function() {
		window.open("${Config.baseURL}bpm/bpm/model/create");
		/* $.openWindow({
			id : "fwebModal_create",
			width : 600,
			height : 280,
...
function image(id) {
	$.openWindow({
		...
		height : 450,
		destroy : true,
		url : basePath + "view/bpm/bpm/config/definitionImg?id=" + id,
		title : "流程图",
		showDefaultButton : false
	});
}
...
function deleteConfig(id) {
	$.confirm("确认删除流程吗", function(flag) {
		if (flag) {
			$.ajax({
				url : basePath + "bpm/bpm/config/resource/" + id,
				type : 'delete',
