	...
	<div class="v_box" id="v_box_header2">
		<div class="widget-header ui-sortable-handle">
			<div class="widget-toolbar no-border" style="z-index: 10000">
				<a href="${Config.baseURL}view/bpm/bpm/config/definitionImg?id=${param.definitionId}" target="_blank">
					<span class="btn btn-xs btn-primary"> <i
						class="ace-icon fa fa-comment-o bigger-130"></i> <span
						class="bigger-110">查看流程图</span>
				</span></a>
				<a href="javascript:void(0)" onclick="history.back()"><span
...
var gridUrl = "bpm/bpm/config/messageConfigList?definitionId=${param.definitionId}";
loadGrid();
function loadGrid(){
				...
				columns : [
						{
							...
							render:function(row){
								var inputHTML = "<input type=\"hidden\" name=\"id\" value=\""+row.id+"\">"
									+ "<input type=\"hidden\" name=\"sortNum_"+row.id+"\" value=\""+row.sortNum+"\"/>"
									+ "<input type=\"hidden\" name=\"messageType_"+row.id+"\" value=\""+row.messageType+"\"/>"
									+ "<input type=\"hidden\" name=\"eventTypeDesc_"+row.id+"\" value=\""+row.eventTypeDesc+"\">"
