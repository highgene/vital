...
&nbsp;<b>配置类型：<select name="type" id="messageType">
...
var gridUrl = "bpm/config/messageList?taskKey="+taskKey+"&type=1";
loadGrid();
function loadGrid(){
...
function saveConfig(id, name, version) {
	$.ajax({
		url : basePath + "bpm/config/messageConfig/",
		type : 'post',
...
function initScopeRoles(){
	...
	$.ajax({
		url : basePath+"bpm/config/roleList/",
		type : "get",
		cache : true,
		contentType : 'application/json;charset=UTF-8',
		success : function(data){
...
function changeType(event){
	...
	gridUrl = "bpm/config/messageList?taskKey="+taskKey+"&type="+type;
...
function initMessageSelect() {
	...
	$.ajax({
		url : basePath+"template/?pageSize=0&pageIndex=1000&templateName=&templateSign=",
		type : "get",
		cache : true,
		contentType : 'application/json;charset=UTF-8',
		success : function(data){
