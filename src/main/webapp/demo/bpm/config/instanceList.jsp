...
function queryProcessinstance(){
	$("#configGrid").dataGrid({
		checkbox:false,
		url:basePath + "bpm/config/instanceList/",
		type:"get",
		pagination:true,
		rownum:false,
		...
		columns:[
			...
			{name:"processDefinitionKey",width:160,title:"流程定义标识"},
			{name:"startTime",width:130,title:"开始时间"},
			{name:"endTime",width:130,title:"结束时间"},
...
function testProcess(definitionId,name,id,businessKey) {
	window.location.href=basePath + "view/demo/bpm/config/testProcess?id=" + definitionId+"&processInstanceId="+id+"&businessKey="+businessKey;
}
...
function forceStop(id){
	$.confirm("确认强制结束流程实例吗？【结束流程后，数据将移入历史库中！】", function(flag){
		if(flag){
			$.ajax({
				url : basePath + "bpm/config/forceStop/" + id,
				type : 'delete',
...
function deleteConfig(id, endTime) {
	...
	$.confirm("确认删除流程实例吗？【删除流程实例将导致相关业务数据的状态失效！】", function(flag){
		if(flag){
			$.ajax({
				url : basePath + "bpm/config/processintances/" + id,
				type : 'delete',
				success : function(data) {
					queryProcessinstance();
				},
